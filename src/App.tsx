...
const S: any = {
  ...
  miniLabel: { fontSize: "9px", color: "#94a3b8", textTransform: "uppercase" as const, fontWeight: "700" },
};
...
export default function App() {
  ...
  useEffect(() => {
    const unsubs = [
      onSnapshot(doc(db, "vinted", "ventes"), (snap) => {
        if (snap.exists()) setVentes(JSON.parse(snap.data().data));
        setLoading(false);
      }),
      onSnapshot(doc(db, "vinted", "stock"), (snap) => { if (snap.exists()) setStock(JSON.parse(snap.data().data)); }),
      onSnapshot(doc(db, "vinted", "vendeurs"), (snap) => {
        if (snap.exists()) { const v = JSON.parse(snap.data().data); setVendeurs(v); setVenteForm(f => ({ ...f, vendeur: v[0]?.nom || "" })); }
        else { setLoading(false); setVenteForm(f => ({ ...f, vendeur: VENDEURS_INIT[0].nom })); }
      }),
  ...
  const save = async (key: string, data: any) => { setSyncing(true); await fbSave(key, data); setTimeout(() => setSyncing(false), 800); };
  ...
  const addStock = async () => {
    ...
    setStock(newStock); await save("stock", newStock);
  ...
  const addPaiement = async () => {
    ...
    setPaiements(newP); await save("paiements", newP);
    setPaiementForm({ montant: "", note: "" }); setShowPaiement(null);
    showToast(`Paiement de ${fmt(+paiementForm.montant)} enregistré ✓`);
  };

  const saveVendeurs = async () => { await save("vendeurs", vendeurs); setShowSettings(false); showToast("Commissions sauvegardées ✓"); };
  ...
  const statsVendeurs = useMemo(() => vendeurs.map((v: any) => {
    ...
    return { ...v, nb: vv.length, nbMonth: vvM.length, ca: vv.reduce((s: number, x: any) => s + x.prixVente, 0), caMonth: vvM.reduce((s: number, x: any) => s + x.prixVente, 0), commissionTotal: totalDu, commissionMonth: vvM.reduce((s: number, x: any) => s + x.commissionMontant, 0), totalPaye, solde: totalDu - totalPaye };
          ...
          <div>
            ...
            {statsVendeurs.filter((v: any) => v.solde > 0).map((v: any) => (
              <div key={v.nom} style={S.card}>
                ...
                <div style={{ fontSize: "12px", color: "#94a3b8", marginTop: "4px" }}>{v.nb} vente(s) · Commission totale : {fmt(v.commissionTotal)}</div>
              </div>
            ))}
            {statsVendeurs.filter((v: any) => v.solde > 0).length === 0 && <div style={{ textAlign: "center", color: "#94a3b8", padding: "30px 0" }}>Tous les vendeurs sont soldés ✅</div>}
      ...
      {showSettings && (
        <div style={S.overlay} onClick={(e: any) => e.target === e.currentTarget && setShowSettings(false)}>
          <div style={S.modal}>
            ...
            <div style={{ marginBottom: "12px", backgroundColor: "#f0fdf4", borderRadius: "12px", padding: "12px", fontSize: "12px", color: "#16a34a", fontWeight: "600" }}>🔄 Synchronisé en temps réel avec ton collaborateur</div>
